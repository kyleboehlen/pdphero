<div class="category">
    @isset($category)
        <form class="category" action="{{ route('goals.destroy.category', ['category' => $category->uuid]) }}" method="POST">
            @csrf
    ...
    @else
        <form class="category" action="{{ route('goals.store.category') }}" method="POST">
            @csrf
