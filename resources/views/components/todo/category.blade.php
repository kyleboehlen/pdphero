<div class="category">
    @isset($category)
        <form class="category" action="{{ route('todo.destroy.category', ['category' => $category->uuid]) }}" method="POST">
            @csrf
    ...
    @else
        <form class="category" action="{{ route('todo.store.category') }}" method="POST">
            @csrf
